...
class pemasukan extends CI_Controller {
	...
	public function tampil_pemasukan() {
		$data['data_pemasukan'] = $this->pemasukan_model->GetPemasukan();
		$this->load->view('pemasukan/tampil',$data);
	}
	...
	public function tambah_pemasukan(){
		...
		insert_log('Menambah Pemasukan Pendapatan');
		$this->pemasukan_model->insert($insert_pemasukan);
		if($saldo=="belum"){
			$masuk=array('saldo_total'=>0,'id_saldo'=>1);
			$this->db->insert('saldo',$masuk);
	...
	public function delete_pemasukan(){
		...
		$jumlah_saldo=$this->pemasukan_model->getjumlah($id_pemasukan);
		...
		$this->saldo_model->update_saldo($insert_saldo);
		$this->pemasukan_model->delete($id_pemasukan);
		
		insert_log('Menghapus Data Pemasukan');
	}
	...
	public function edit_pemasukan(){
		...
		$jumlah_pemasukan=$this->input->post('jumlah_pemasukan');
		$saldo_sekarang=$this->pemasukan_model->getjumlah($id_pemasukan);
		...
		insert_log('Mengubah Pemasukan Pendapatan');
		$this->pemasukan_model->update_saldo($insert_pemasukan,$id_pemasukan);
		$this->saldo_model->update_saldo($insert_saldo);
	}
}
